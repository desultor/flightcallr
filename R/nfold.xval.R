...
#' 
#' @param measurements A set of selection table rows with Call_vs_Noise column 
#'   filled in, and with features as generated by generate.seewave.measures()
#' @param annotation An optional annotation for the plots.
#' @param fold n, i.e. how many rounds of cross-validation to perform.
#' @param plot.ROC Whether or not to plot the ROC curve pertaining to the 
#'   cross-validation run.
#' @param color.plot Whether or not the plot should be in color.
#' @return \item{name perf}{description a perf object}
#'         \item{name difficulty}{description a vector signifying how 
#'           frequently the row in question is incorrectly identified in 
#'           cross-validation}
nfold.xval = function(
measurements, 
annotation="", 
...
color.plot=T
  ...
  for (i in 1:fold) {
    ...
    rf = randomForest::randomForest(Call_vs_Noise ~ ., data=splits$trainset[, c(seewave.measures, "Call_vs_Noise", "Detector")], 
